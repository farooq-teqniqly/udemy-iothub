...
namespace AzCourse.DPS.Sample
{
    internal class DPSGroupSample
    {
        ...
        const string PRIMARY_KEY = "xxxxx";
        const string ID_SCOPE = "xxxx";

        // The following consts should not be changed
        const string DESIRED_DEVICE_ID = "iot-dev-dps-01";
        ...
        public async Task RunSampleAsync()
        {
            ...
            var derivedKey = ComputeDerivedSymmetricKey(
                Convert.FromBase64String(PRIMARY_KEY),
                DESIRED_DEVICE_ID
            );
        ...
        private string ComputeDerivedSymmetricKey(byte[] masterKey, string registrationId)
        {
            using (var hmac = new HMACSHA256(masterKey))
            {
                return Convert.ToBase64String(
                    hmac.ComputeHash(Encoding.UTF8.GetBytes(registrationId))
                );
            }
        }
    }
